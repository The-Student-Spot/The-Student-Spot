...
import {
  Bell, School, Briefcase, Handshake, Calendar, Users, LogOut, ChevronRight
} from "lucide-react";
import { NavLink, useNavigate } from "react-router-dom";
import { motion } from "framer-motion";

const dashboardItems = [
    { icon: <Briefcase />, title: "Placement Support", description: "Manage and track student placements." },
    { icon: <Handshake />, title: "Industry Collaborations", description: "Partner with companies for projects." },
    { icon: <Calendar />, title: "Events & Hackathons", description: "Organize and promote events." },
    { icon: <Users />, title: "Partner with Companies", description: "Build relationships with recruiters." },
    { icon: <Briefcase />, title: "College Job Board", description: "Post and manage job listings." },
];
    ...
    <div className="grid min-h-screen w-full lg:grid-cols-[280px_1fr] bg-slate-50">
      <aside className="hidden border-r bg-card lg:block">
        <div className="flex h-full max-h-screen flex-col">
          ...
          <nav className="flex-1 overflow-auto py-4 px-4 text-sm font-medium space-y-1">
            <NavLink to="#" className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-muted-foreground transition-all hover:text-yellow-600 hover:bg-yellow-50"><Briefcase className="h-4 w-4" />Placement Support</NavLink>
            <NavLink to="#" className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-muted-foreground transition-all hover:text-yellow-600 hover:bg-yellow-50"><Handshake className="h-4 w-4" />Collaborations</NavLink>
      ...
      <main className="flex flex-col flex-1 gap-6 p-4 lg:p-8">
        <header className="flex items-center justify-between">
           <h1 className="text-2xl font-bold text-slate-800">Welcome, College Partner!</h1>
