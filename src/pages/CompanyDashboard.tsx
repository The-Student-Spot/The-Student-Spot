...
import {
  Bell, Building, Users, Briefcase, Handshake, LogOut, ChevronRight
} from "lucide-react";
import { NavLink, useNavigate } from "react-router-dom";
import { motion } from "framer-motion";

const dashboardItems = [
    { icon: <Users />, title: "Hire Interns / Freshers", description: "Find top talent for your team." },
    { icon: <Briefcase />, title: "Post Jobs & Projects", description: "Share opportunities with students." },
    { icon: <Handshake />, title: "Branding & Engagement", description: "Promote your brand on campus." },
    { icon: <Users />, title: "Access Student Talent Pool", description: "Search and filter student profiles." },
];
    ...
    <div className="grid min-h-screen w-full lg:grid-cols-[280px_1fr] bg-slate-50">
      <aside className="hidden border-r bg-card lg:block">
        <div className="flex h-full max-h-screen flex-col">
          ...
          <nav className="flex-1 overflow-auto py-4 px-4 text-sm font-medium space-y-1">
            <NavLink to="#" className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-muted-foreground transition-all hover:text-yellow-600 hover:bg-yellow-50"><Users className="h-4 w-4" />Talent Pool</NavLink>
            <NavLink to="#" className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-muted-foreground transition-all hover:text-yellow-600 hover:bg-yellow-50"><Briefcase className="h-4 w-4" />Jobs & Projects</NavLink>
            <NavLink to="#" className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-muted-foreground transition-all hover:text-yellow-600 hover:bg-yellow-50"><Handshake className="h-4 w-4" />Engagement</NavLink>
          </nav>
      ...
      <main className="flex flex-col flex-1 gap-6 p-4 lg:p-8">
        <header className="flex items-center justify-between">
           <h1 className="text-2xl font-bold text-slate-800">Welcome, Company Partner!</h1>
