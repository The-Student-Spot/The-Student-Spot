...
const StudentDashboardNavLayout = ({
  ...
  return (
    <div className="flex h-screen w-full flex-col bg-slate-50">
      {/* FIXED TOP NAVBAR - Static Branding */}
      ...
      <div className="flex flex-1 overflow-hidden pt-20">
        {/* Desktop Sidebar - Collapsible Menu Only */}
        <aside
        ...
        >
          {sidebarOpen && (
            <>
              ...
              <nav className="flex-1 overflow-auto py-4 px-3 text-sm font-medium space-y-1">
                {navItems.map((item) => (
                  <button
                    key={item.id}
                    type="button"
        ...
        <button
          ...
          className="hidden lg:flex items-center justify-center w-10 border-r border-slate-200 bg-slate-50 hover:bg-slate-100 transition-colors flex-shrink-0"
          aria-label={sidebarOpen ? "Collapse sidebar" : "Expand sidebar"}
        >
          <ChevronLeft className={`h-5 w-5 text-slate-400 transition-transform ${sidebarOpen ? "" : "rotate-180"}`} />
        </button>
